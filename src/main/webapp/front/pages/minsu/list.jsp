                    ...
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="minsuName" id="minsuName" placeholder="民宿名称" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <select name="fagwuTypes" id="fagwuTypes" lay-filter="fagwuTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in fagwuTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="minsuNewMoneyStart" id="minsuNewMoneyStart"  lay-skin="double"   placeholder="价格/天开始" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <div class="layui-input-inline" style="margin-top: -5px">
                              <input type="text" name="minsuNewMoneyEnd"   id="minsuNewMoneyEnd"    lay-skin="double|minsuNewMoneyEnd"   placeholder="价格/天结束" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <select name="fwstateTypes" id="fwstateTypes" lay-filter="fwstateTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in fwstateTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../minsu/detail.jsp?id='+item.id)">
                                        <a href="#" class="ng-prsliik">
                                            <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                                <img class="swiper-item" :src="item.minsuPhoto?item.minsuPhoto.split(',')[0]:''" />
                                            </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../minsu/detail.jsp?id='+item.id)"><span>{{item.minsuName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.minsuNewMoney"><span style="color: red;">{{item.minsuNewMoney}} RMB</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    ...
    // 当前表的字典表
        ...
        function fagwuTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=fagwu_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.fagwuTypesList = res.data.list;
                }
            });
        }
        // 房屋状态的查询
        function fwstateTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=fwstate_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.fwstateTypesList = res.data.list;
                }
            });
        }
        ...
        form.verify({
            ...
            ,minsuNewMoneyEnd: function(value, item){ //value：表单的值、item：表单的DOM对象
                var minsuNewMoneyStart = $("#minsuNewMoneyStart").val();
                if(minsuNewMoneyStart != null && minsuNewMoneyStart != "" && minsuNewMoneyStart != "null"){
                    var number = parseInt(value)- parseInt(minsuNewMoneyStart);
                    if(number<0){
                        return '价格/天的结束值不能小于开始值';
                    }
                }
            }
        ...
        function pageList() {
            ...
            if (jquery('#minsuName').val() != null && jquery('#minsuName').val() != "" && jquery('#minsuName').val() != "null" ) {
                param['minsuName'] = jquery('#minsuName').val();
            }
         
            var fagwuTypes = $("#fagwuTypes").find("option:selected").val();
            if (fagwuTypes != null && fagwuTypes != "" && fagwuTypes != "null") {
                param['fagwuTypes'] = fagwuTypes;
            }
         
            if (jquery('#minsuNewMoneyStart').val() != null && jquery('#minsuNewMoneyStart').val() != "" && jquery('#minsuNewMoneyStart').val() != "null" ) {
                param['minsuNewMoneyStart'] = jquery('#minsuNewMoneyStart').val();
            }
            if (jquery('#minsuNewMoneyEnd').val() != null && jquery('#minsuNewMoneyEnd').val() != "" && jquery('#minsuNewMoneyEnd').val() != "null" ) {
                param['minsuNewMoneyEnd'] = jquery('#minsuNewMoneyEnd').val();
            }
                         
            var fwstateTypes = $("#fwstateTypes").find("option:selected").val();
            if (fwstateTypes != null && fwstateTypes != "" && fwstateTypes != "null") {
                param['fwstateTypes'] = fwstateTypes;
            }
                        
            // 获取列表数据
            http.request('minsu/list', 'get', param, function(res) {
